...
export default function NewOrderPage() {
  ...
  const [technicians, setTechnicians] = useState<(Sel & { role: string })[]>([])
  ...
  useEffect(() => {
    ...
    supabase.from('profiles').select('id, full_name, role')
      ...
      .then(({ data }) => setTechnicians(
        (data ?? []).map((p) => ({ id: p.id, name: p.full_name, role: p.role }))
      ))
  }, [])

  // Cargar sedes cuando cambia el cliente
  useEffect(() => {
    setSiteId('')
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5 px-4 py-5">
        ...
        <Field label="Sede *">
          <select
            ...
            required
            disabled={!customerId}
            className={cn(selectCls, !customerId && 'opacity-50')}
          >
            <option value="">
              {customerId ? 'Seleccionar sede...' : 'Primero elige un cliente'}
            </option>
            {sites.map((s) => (
          ...
          {customerId && sites.length === 0 && (
            <p className="text-xs text-ink-tertiary mt-1">
              Este cliente no tiene sedes. <Link href="/customers" className="text-volt-500 underline">Agrégalas desde Clientes</Link>.
            </p>
          )}
        </Field>
          ...
          >
            ...
            {technicians.map((t) => (
              <option key={t.id} value={t.id}>
                {t.name} ({t.role === 'supervisor' ? 'Supervisor' : 'Técnico'})
              </option>
            ))}
          </select>
        </Field>
